    ...
    <div className={`${theme.card} border ${theme.border} rounded-xl p-5 shadow-sm`}>
      <div className="mb-4">
        ...
        <p className={`text-sm ${theme.textMuted}`}>
          Product categories with elevated flagged or mismatch rates
        </p>
      </div>

      <div className="space-y-3">
        {categories.length === 0 ? (
          <p className={`text-sm ${theme.textMuted}`}>No category risk data available.</p>
        ) : (
          categories.map((item) => (
            <div key={item.category} className={`border ${theme.border} rounded-xl p-4`}>
              <div className="flex items-start justify-between gap-3 mb-2">
                <div>
                  <h4 className="font-medium">{item.category}</h4>
                  <p className={`text-sm ${theme.textMuted}`}>
                    {item.flagged} flagged out of {item.total} records
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between text-sm mb-2">
                ...
                <span className="font-semibold">{item.flaggedRate.toFixed(1)}%</span>
              </div>
              ...
              <div className="w-full h-2 rounded-full bg-gray-200 overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(item.flaggedRate, 100)}%` }}
                />
              </div>
            </div>
          ))
